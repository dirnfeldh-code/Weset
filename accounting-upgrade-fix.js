(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .vat-payment-row span,
      .vat-summary-row span {
        min-width: 0;
        overflow-wrap: anywhere;
      }
      .expense-category-row button,
      .vat-payment-row button {
        flex: 0 0 auto;
      }
      ...
      .vat-summary-row.is-total {
        background: #e4f2ed;
        border-color: rgba(57, 181, 74, 0.3);
      }
      .vat-summary-row.is-warning {
        background: #fff7ed;
        border-color: #fed7aa;
      }
      .expense-editing-note {
        background: #e8f3f1;
        border: 1px solid rgba(20, 92, 88, 0.24);
        border-radius: 8px;
        color: #145c58;
        display: none;
        font-weight: 700;
      ...
      .expense-editing-note.is-visible {
        display: block;
      }
      #cancelExpenseEditBtn {
        display: none;
      }
      #cancelExpenseEditBtn.is-visible {
        display: inline-flex;
      }
  ...
  function populateExpenseCategories(selected = "") {
    ...
    if (current && !allCategories().includes(current)) {
      els.expenseCategory.insertAdjacentHTML("beforeend", `<option value="${escapeHtml(current)}">${escapeHtml(current)}</option>`);
    }
  ...
  function renderExpensesTableWithEdit() {
    ...
    els.expensesTable.innerHTML = [...(state.expenses || [])].sort((a, b) => String(b.date || "").localeCompare(String(a.date || ""))).map((expense) => `<tr>
      <td>${dateText(expense.date)}</td>
      <td>${escapeHtml(expense.category || "Other")}</td>
      <td>${escapeHtml(expense.payee || "")}${expense.notes ? `<p class="meta">${escapeHtml(expense.notes)}</p>` : ""}</td>
      <td><strong>${moneyText(expense.amount || 0)}</strong></td>
      <td><div class="expense-row-actions"><button class="secondary" data-edit-expense="${escapeHtml(expense.id)}" type="button">Edit</button><button class="ghost danger" data-delete-expense="${escapeHtml(expense.id)}" type="button">Delete</button></div></td>
    </tr>`).join("") || `<tr><td colspan="5"><div class="empty">No expenses recorded yet.</div></td></tr>`;
  }

  function categoryIsUsed(category) {
    return (state.expenses || []).some((expense) => expense.category === category);
  }
  ...
  function ensureCategoryManager() {
    ...
    panel.innerHTML = `<section class="panel" id="expenseCategoryManager">
      <div class="panel-head"><h2>Expense categories</h2></div>
      <form class="expense-category-form" id="expenseCategoryForm">
        <input id="newExpenseCategory" placeholder="New category name" aria-label="New expense category">
        <button class="secondary" type="submit">Add category</button>
      </form>
      <div class="expense-category-list" id="expenseCategoryList"></div>
    </section>
    <section class="panel" id="vatSummaryPanel">
      <div class="panel-head"><div><h2>VAT summary</h2><p class="meta">Estimated from invoices, quotes and expenses in the app.</p></div></div>
      <div class="vat-summary-list" id="vatSummaryList"></div>
      <form class="vat-payment-form" id="vatPaymentForm">
        <input id="vatPaymentDate" type="date" aria-label="VAT payment date">
        <input id="vatPaymentAmount" min="0" step="0.01" type="number" placeholder="Amount paid" aria-label="VAT amount paid">
        <button class="secondary" type="submit">Record VAT paid</button>
      </form>
      <div class="vat-payment-list" id="vatPaymentList"></div>
    </section>`;
  ...
  function renderCategoryManager() {
    ...
    list.innerHTML = custom.map((category) => {
  ...
  function renderVatSummary() {
    ...
    const netDue = Math.max(0, salesVat - inputVat);
    ...
    list.innerHTML = `<div class="vat-summary-row"><span>VAT collected on sales</span><strong>${moneyText(salesVat)}</strong></div>
      <div class="vat-summary-row"><span>Estimated VAT on expenses</span><strong>${moneyText(inputVat)}</strong></div>
      <div class="vat-summary-row"><span>Net VAT to pay</span><strong>${moneyText(netDue)}</strong></div>
      <div class="vat-summary-row"><span>VAT already paid</span><strong>${moneyText(paid)}</strong></div>
      <div class="vat-summary-row ${remaining > 0 ? "is-warning" : "is-total"}"><span>Remaining VAT to pay</span><strong>${moneyText(remaining)}</strong></div>
      <p class="meta">Expense VAT is estimated as 20% VAT included in expense totals. Your accountant can adjust the real figures.</p>`;
    paymentList.innerHTML = vatPayments().map((payment) => `<div class="vat-payment-row"><span>${dateText(payment.date)} - ${moneyText(payment.amount)}</span><button class="ghost danger" data-delete-vat-payment="${escapeHtml(payment.id)}" type="button">Delete</button></div>`).join("") || `<div class="empty">No VAT payments recorded yet.</div>`;
  }
  ...
  function addVatPayment(event) {
    ...
    if (amount <= 0) return alert("Enter the VAT amount you paid.");
    saveVatPayments([{ id: crypto.randomUUID(), date: dateInput.value || new Date().toISOString().slice(0, 10), amount }, ...vatPayments()]);
    amountInput.value = "";
    renderVatSummary();
  }

  function deleteVatPayment(id) {
    saveVatPayments(vatPayments().filter((payment) => payment.id !== id));
    renderVatSummary();
  }
  ...
  if (oldRenderAccounting) {
    renderAccounting = function renderAccountingWithUpgrade() {
      oldRenderAccounting();
  ...
  document.addEventListener("submit", (event) => {
    if (event.target?.matches?.("#expenseForm") && editingExpenseId) {
      saveEditedExpense(event).catch((error) => alert(`Could not update expense: ${error.message || "Please check Supabase and try again."}`));
    }
  }, true);
  ...
  document.addEventListener("click", (event) => {
    ...
    if (editButton) {
      event.preventDefault();
      event.stopPropagation();
      editExpense(editButton.dataset.editExpense);
    }
    if (deleteCategoryButton) {
      event.preventDefault();
      event.stopPropagation();
      deleteExpenseCategory(deleteCategoryButton.dataset.deleteCategory);
    }
    if (deleteVatPaymentButton) {
      event.preventDefault();
      event.stopPropagation();
      deleteVatPayment(deleteVatPaymentButton.dataset.deleteVatPayment);
    }
  }, true);
