(() => {
  ...
  function companyDetails() {
    if (typeof window.wesetGetCompanyDetails === "function") return { ...fallbackCompany, ...window.wesetGetCompanyDetails() };
    return { ...fallbackCompany, ...(window.wesetCompanyDetails || {}) };
  }
  ...
  function itemRows(quote) {
    return quoteItemsSafe(quote).map((item, index) => {
      ...
      return { index: index + 1, product: item.name || "Services", description: itemDescription(item, quote), qty, rate, amount: qty * rate };
    });
  }
  ...
  function documentHtml(kind, quote) {
    ...
    return `<div style="font-family:Arial,Helvetica,sans-serif;color:#1d2528;background:#ffffff;max-width:900px;margin:0 auto;padding:0;">
      <div style="display:grid;grid-template-columns:1.15fr .85fr;gap:24px;padding:42px 48px 22px;align-items:start;">
        <div><h1 style="color:#0d6f9f;font-size:20px;letter-spacing:2px;margin:0 0 8px;">${escapeHtml(title)}</h1><p style="font-size:12px;line-height:1.45;margin:0;"><strong>${escapeHtml(company.name)}</strong><br>${escapeHtml(company.address1)}<br>${escapeHtml(company.address2)}<br>${escapeHtml(company.vat)}</p></div>
        <div style="display:grid;grid-template-columns:1fr;gap:10px;justify-items:end;"><p style="font-size:12px;line-height:1.45;margin:0;text-align:left;width:100%;max-width:250px;">${escapeHtml(company.email)}<br>${escapeHtml(company.phone)}</p><div style="background:#eeeeee;border-radius:2px;padding:12px 18px;width:260px;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.12);"><img src="${escapeHtml(company.logoUrl || fallbackCompany.logoUrl)}" alt="WeSet" style="display:block;width:100%;height:auto;"></div></div>
      </div>
      <div style="background:#eaf3fb;padding:22px 48px 28px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:70px;margin-bottom:34px;"><div><h2 style="font-size:12px;margin:0 0 6px;">Bill to</h2><p style="font-size:13px;line-height:1.45;margin:0;">${escapeHtml(client.company || client.contact || "Client")}<br>${escapeHtml(client.contact || "")}<br>${escapeHtml(client.email || "")}</p></div><div><h2 style="font-size:12px;margin:0 0 6px;">Ship to</h2><p style="font-size:13px;line-height:1.45;margin:0;">${escapeHtml(client.company || client.contact || "Client")}<br>${escapeHtml(quote.premises || "")}</p></div></div>
        <div style="border-top:1px dashed #ccd7df;padding-top:24px;max-width:280px;"><h2 style="font-size:13px;margin:0 0 8px;">${escapeHtml(kind)} details</h2><p style="font-size:13px;line-height:1.55;margin:0;">${escapeHtml(kind)} no.: ${escapeHtml(reference)}<br>Terms: ${escapeHtml(company.terms || "Net 15")}<br>${escapeHtml(kind)} date: ${escapeHtml(date)}<br>Due date: ${escapeHtml(dueDate)}</p></div>
      </div>
      <div style="padding:30px 48px 34px;"><table style="border-collapse:collapse;width:100%;font-size:12px;"><thead><tr><th style="padding:10px 8px;text-align:left;width:30px;">#</th><th style="padding:10px 8px;text-align:left;width:27%;">Product or service</th><th style="padding:10px 8px;text-align:left;">Description</th><th style="padding:10px 8px;text-align:right;width:55px;">Qty</th><th style="padding:10px 8px;text-align:right;width:85px;">Rate</th><th style="padding:10px 8px;text-align:right;width:95px;">Amount</th><th style="padding:10px 8px;text-align:right;width:75px;">VAT</th></tr></thead><tbody>${rows.map((row) => `<tr><td style="border-top:1px solid #e2e6e8;padding:13px 8px;vertical-align:top;">${row.index}.</td><td style="border-top:1px solid #e2e6e8;padding:13px 8px;vertical-align:top;font-weight:700;line-height:1.45;">${escapeHtml(row.product)}</td><td style="border-top:1px solid #e2e6e8;padding:13px 8px;vertical-align:top;line-height:1.45;">${escapeHtml(row.description)}</td><td style="border-top:1px solid #e2e6e8;padding:13px 8px;text-align:right;vertical-align:top;">${row.qty}</td><td style="border-top:1px solid #e2e6e8;padding:13px 8px;text-align:right;vertical-align:top;">${moneyText(row.rate)}</td><td style="border-top:1px solid #e2e6e8;padding:13px 8px;text-align:right;vertical-align:top;">${moneyText(row.amount)}</td><td style="border-top:1px solid #e2e6e8;padding:13px 8px;text-align:right;vertical-align:top;">${totals.vatEnabled ? `${totals.vatRate}% S` : "No VAT"}</td></tr>`).join("") || `<tr><td colspan="7" style="border-top:1px solid #e2e6e8;padding:13px 8px;">Office setup services</td></tr>`}</tbody></table>
        <div style="display:grid;grid-template-columns:1fr 270px;gap:28px;margin-top:18px;align-items:start;"><div></div><div style="font-size:13px;"><div style="display:flex;justify-content:space-between;border-top:1px solid #e2e6e8;padding:10px 0;"><span>Subtotal</span><span>${moneyText(totals.subtotal)}</span></div><div style="display:flex;justify-content:space-between;border-top:1px solid #e2e6e8;padding:10px 0;"><span>Includes VAT @ ${totals.vatEnabled ? totals.vatRate : 0}%</span><span>${moneyText(totals.vatAmount)}</span></div><div style="display:flex;justify-content:space-between;border-top:1px solid #d2d9dd;padding:16px 0 0;font-size:16px;font-weight:800;"><span>Total</span><span>${moneyText(totals.total)}</span></div></div></div>
      </div>
    </div>`;
  }
  ...
  function structuredText(kind, quote) {
    ...
    const totals = totalsForQuote(quote);
    const reference = kind === "Invoice" ? invoiceNumber(quote) : quoteRef(quote);
    return [`DOC_TYPE: ${kind}`, `REFERENCE: ${reference}`, `QUOTE_REF: ${quoteRef(quote)}`, `DATE: ${new Date().toISOString().slice(0, 10)}`, `DUE_DATE: ${quote.requiredDate || ""}`, `FROM_COMPANY: ${company.name}`, `FROM_ADDRESS1: ${company.address1}`, `FROM_ADDRESS2: ${company.address2}`, `FROM_VAT: ${company.vat}`, `FROM_EMAIL: ${company.email}`, `FROM_PHONE: ${company.phone}`, `TERMS: ${company.terms || "Net 15"}`, `CLIENT_COMPANY: ${client.company || client.contact || "Client"}`, `CLIENT_CONTACT: ${client.contact || ""}`, `CLIENT_EMAIL: ${client.email || ""}`, `SHIP_TO: ${quote.premises || ""}`, ...itemRows(quote).map((row) => `ITEM: ${row.product} | DESC: ${row.description} | QTY: ${row.qty} | RATE: ${moneyText(row.rate)} | AMOUNT: ${moneyText(row.amount)} | VAT: ${totals.vatEnabled ? `${totals.vatRate}% S` : "No VAT"}`), `SUBTOTAL: ${moneyText(totals.subtotal)}`, `VAT_LABEL: Includes VAT @ ${totals.vatEnabled ? totals.vatRate : 0}%`, `VAT_AMOUNT: ${moneyText(totals.vatAmount)}`, `TOTAL: ${moneyText(totals.total)}`, "", `Hello ${client.contact || ""},`, `Your ${kind.toLowerCase()} ${reference} is ready. The PDF is attached.`, "Kind regards,", company.name || "WeSet"].join("\n");
  }
  ...
  window.wesetQuoteEmailPayload = (quote) => {
    ...
    return { to: client.email || "", subject: `Quote ${reference}`, text: structuredText("Quote", quote), html: designedEmail("Quote", quote, { reference }), reference, invoiceHtml: documentHtml("Quote", quote) };
  };
  window.wesetInvoiceEmailPayload = (quote) => {
    const client = getClientSafe(quote);
    const reference = invoiceNumber(quote);
    return { to: client.email || "", subject: `Invoice ${reference}`, text: structuredText("Invoice", quote), html: designedEmail("Invoice", quote, { reference }), reference, invoiceHtml: documentHtml("Invoice", quote) };
  };
