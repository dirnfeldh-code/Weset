(() => {
  ...
  style.textContent = `
    ...
    dialog .dialog-card { max-height: min(96dvh, calc(100vh - 10px)) !important; overflow: auto !important; overscroll-behavior: contain !important; padding: 16px !important; width: 100% !important; }
    dialog .panel-head:first-child { background: #fff !important; border-bottom: 1px solid var(--line, #d9e0e1) !important; margin: -16px -16px 12px !important; padding: 12px 16px !important; position: sticky !important; top: -16px !important; z-index: 5 !important; }
    dialog .dialog-actions { background: #fff !important; border-top: 1px solid var(--line, #d9e0e1) !important; bottom: -16px !important; margin: 8px -16px -16px !important; padding: 12px 16px !important; position: sticky !important; z-index: 4 !important; }
    dialog .email-preview { max-height: 46dvh !important; overflow: auto !important; }
    dialog #quoteEmailText { min-height: 140px !important; max-height: 26dvh !important; }
    .send-in-app-button { background: #145c58 !important; color: #fff !important; }
    .send-in-app-button[disabled] { cursor: wait !important; opacity: 0.72 !important; }
    @media (max-width: 700px) {
      dialog { border-radius: 0 !important; height: 100dvh !important; max-height: 100dvh !important; max-width: 100vw !important; width: 100vw !important; }
      dialog .dialog-card, dialog .dialog-card.wide, dialog .dialog-card.client-dialog-card, dialog .dialog-card.email-dialog { border-radius: 0 !important; grid-template-columns: 1fr !important; height: 100dvh !important; max-height: 100dvh !important; width: 100vw !important; }
      dialog .dialog-card .form-grid, dialog .dialog-card.wide .form-grid, dialog .dialog-card.client-dialog-card, dialog .address-grid, dialog .permission-grid, dialog .selected-item { grid-template-columns: 1fr !important; }
      dialog .span-2, dialog .dialog-card.client-dialog-card .address-builder, dialog .dialog-card.client-dialog-card button[type="submit"] { grid-column: 1 / -1 !important; }
      dialog .dialog-actions, dialog .card-actions { align-items: stretch !important; flex-direction: column !important; }
      dialog .dialog-actions > *, dialog .card-actions > * { width: 100% !important; }
    }
  `;
  ...
  function fallbackEmail(quote) {
    ...
    return { to: client.email || "", subject, text, html: "", reference };
  }

  function quoteEmail(quote) {
    const client = typeof getClient === "function" ? getClient(quote.clientId) : {};
    const payload = typeof window.wesetQuoteEmailPayload === "function" ? window.wesetQuoteEmailPayload(quote) : fallbackEmail(quote);
  ...
  async function sendQuoteInApp(id, button = null) {
    ...
    try {
      const response = await fetch(edgeFunctionUrl, {
        ...
        body: JSON.stringify({ quoteId: quote.id, to: email.to, subject: email.subject, text: email.text, html: email.html, reference: email.reference, invoiceHtml: typeof window.wesetQuoteInvoiceHtml === "function" ? window.wesetQuoteInvoiceHtml(quote) : email.html })
      });
      ...
      alert(`Could not send straight from the app yet: ${error.message || "Email service is not configured."}\n\nIf you just updated the Edge Function, redeploy it in Supabase so it can accept designed HTML and attachments.`);
    } finally {
  ...
  if (oldRenderQuotes) renderQuotes = function renderQuotesWithEmail() { oldRenderQuotes(); addDirectEmailButtons(); enhanceEmailDialog(); };
  const oldRenderDashboard = typeof renderDashboard === "function" ? renderDashboard : null;
  if (oldRenderDashboard) renderDashboard = function renderDashboardWithEmail() { oldRenderDashboard(); addDirectEmailButtons(); enhanceEmailDialog(); };
  ...
  if (oldShowQuoteEmail) showQuoteEmail = function showQuoteEmailAndEnhance(id) { oldShowQuoteEmail(id); enhanceEmailDialog(); };
