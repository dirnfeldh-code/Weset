(() => {
  ...
  function quoteRef(invoice) {
    ...
    if (quote?.id && typeof window.wesetQuoteReference === "function") return window.wesetQuoteReference(quote);
    if (quoteId && typeof window.wesetQuoteReference === "function") return window.wesetQuoteReference(quoteId);
  ...
  function invoiceNumber(invoice) {
    ...
    const quoteId = invoice.quoteId || invoice.quote_id || "";
    if ((!current || isUuid(current) || /^INV-[0-9a-f-]{20,}$/i.test(String(current))) && quoteId && typeof window.wesetInvoiceReferenceForQuote === "function") {
      return window.wesetInvoiceReferenceForQuote(quoteId);
    }
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .weset-expanded-backdrop {
        background: rgba(11, 24, 28, .38) !important;
      }
      dialog::backdrop,
      ...
      .app-settings-dialog::backdrop {
        background: rgba(11, 24, 28, .42) !important;
      }
      .weset-expanded-window,
      dialog.weset-expanded-window {
        background: #fff !important;
        box-shadow: 0 20px 58px rgba(10, 24, 28, 0.22) !important;
      ...
      #accountingWorkspaceShell .panel.accounting-inline-expanded,
      #accountingWorkspaceShell .accounting-tools-grid.accounting-inline-expanded,
      #accountingWorkspaceShell .accounting-grid.accounting-inline-expanded {
        grid-column: 1 / -1;
      }
      #accountingWorkspaceShell .panel.accounting-inline-expanded .report-table-wrap,
      #accountingWorkspaceShell .panel.accounting-inline-expanded .table-wrap,
      #accountingWorkspaceShell .panel.accounting-inline-expanded .expense-category-list,
      #accountingWorkspaceShell .panel.accounting-inline-expanded .vat-payment-list,
      #accountingWorkspaceShell .panel.accounting-inline-expanded .vat-summary-list {
        max-height: none !important;
        overflow: auto !important;
      }
  ...
  function addAccountingExpanders() {
    document.querySelectorAll("#accountingWorkspaceShell .panel").forEach((panel) => {
      if (panel.id === "accountingCreateInvoicePanel") return;
